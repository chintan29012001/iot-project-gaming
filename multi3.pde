...
int PORT_RX=5000;
String HOST_IP = "192.168.61.35";//IP Address of the PC in which this App is running
String Receiver="192.168.61.29";//ip address of multiplayer
...
void setup()
{
  ...
  udp.log(true);
  udp.listen(true);
  udp.send("0","192.168.61.29");
  p=int(random(2));
  if(p==0)
  {
    udp.send("1","192.168.61.29");
  }
  else if(p==1)
  {
    udp.send("0","192.168.61.29");
...
{// if(xPos>sc+200 || ((sc>=800)&&(xPos+1000>sc+200)) )
   //{fr();
   //sc=0;}
   int secn = second() + minute()*60;
  
  if(int(secn-sec)%time==0)
  {
...
if(mo==6)
{
  ...
  {
    udp.send(str(score),"192.168.61.29");
    go=1;
  //print(score);
  score=score-1;
  xPos=xPos-1;
...
if(x1[7] >= 0 && x1[7] <= 7){
 ...
 //if(x1[7]>4){ frameRate(175);}
   //if(x1[7]<=4){ frameRate(300);}
  }
